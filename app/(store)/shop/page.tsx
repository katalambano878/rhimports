...
function ShopContent() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        const cacheKey = `shop:v2:${selectedCategory}:${search || ''}:${priceRange.join('-')}:${selectedRating}:${sortBy}:${page}`;
        ...
        const { data, count, error } = await cachedQuery<{ data: any; count: any; error: any }>(
          ...
          async () => {
            ...
            if (selectedCategory !== 'all') {
              ...
              if (categoryObj) {
                const targetCategoryIds = [categoryObj.id];
                const childCategoryIds = categories
                  .filter(c => c.parent_id === categoryObj.id)
                  .map(c => c.id);
                targetCategoryIds.push(...childCategoryIds);
                query = query.in('category_id', targetCategoryIds);
              } else {
                // Fallback when categories are not loaded yet: filter by joined slug
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="lg:hidden bg-white border-b border-gray-200 py-3 px-4 sticky top-14 z-20 shadow-sm">
